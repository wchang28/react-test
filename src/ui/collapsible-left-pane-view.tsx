...
const getToggleArea = (collapsed: boolean, onToggleClicked: () => void) => {
    ...
    return (
        <div className={`${CLASS_PREFIX}-toggle-area`} style={{float:"right", height: `${TOGGLE_AREA_DIM_PX}px`, width: `${TOGGLE_AREA_DIM_PX}px`, position: "relative"}}>
...
export default (props: ReactProps<Props>) => {
    ...
    return (
        <div className={`${CLASS_PREFIX}-main w3-bar`}>
            <div className={`${CLASS_PREFIX}-left`} style={{float:"left", width:`${leftWidth}px`}}>
                {getLeftPane(collapsed, onToggleClicked, leftPaneContent)}
            </div>
            <div className={`${CLASS_PREFIX}-right`} style={{marginLeft:`${leftWidth}px`}}>
